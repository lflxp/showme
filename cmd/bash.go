...
var (
	bashCompletion = `__showme-cli_debug()
{
...
_k() {
	...
	# 获取一次性结果
	result=$(command showme smart "${COMP_LINE}")

	__showme-cli_debug "result ${result}"
...
`
	...
	bashCompletionDebug = `__showme-cli_debug()
{
	local file="$BASH_COMP_DEBUG_FILE"
	if [[ -n ${file} ]]; then
		echo "$*" >> "${file}"
...
_k() {
	...
	# 获取一次性结果
	result=$(command showme smart -d "${COMP_LINE}")
...
func init() {
	...
	bashCmd.Flags().BoolVarP(&zshdebugLevel, "debug", "d", false, "是否开启debug日志")
}
