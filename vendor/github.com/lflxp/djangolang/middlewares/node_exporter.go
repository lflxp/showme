...
var (
	...
	disableExporterMetrics   bool   = false
	maxRequests              int    = 40
	disableDefaultCollectors bool   = false
	maxProcs                 int    = runtime.NumCPU()
	toolkitFlags             string = ":9100"
)
	...
	if h.includeExporterMetrics {
		handler = promhttp.HandlerFor(
			...
			promhttp.HandlerOpts{
				ErrorLog:            stdlog.New(log.NewStdlibAdapter(level.Error(h.logger)), "", 0), // 采集过程中如果出现错误，记录日志
...
func prometheusHandler() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		newHandler(!disableExporterMetrics, maxRequests, logger).ServeHTTP(c.Writer, c.Request)
	}
}
